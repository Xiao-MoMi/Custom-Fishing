...
public class ItemManagerImpl implements ItemManager, Listener {
    ...
    private final HashMap<String, ItemProvider> itemLibraryMap;
    private ItemProvider[] itemDetectionArray;
    private NamespacedKey blockKey;

    public ItemManagerImpl(BukkitCustomFishingPlugin plugin) {
        instance = this;
        this.plugin = plugin;
    ...
    private void resetItemDetectionOrder() {
        ...
        for (String plugin : CFConfig.itemDetectOrder) {
            ItemProvider library = itemLibraryMap.get(plugin);
            if (library != null) {
                list.add(library);
            }
        }
        this.itemDetectionArray = list.toArray(new ItemProvider[0]);
    }
    ...
    public String getAnyPluginItemID(ItemStack itemStack) {
        if (itemStack == null || itemStack.getType() == Material.AIR)
            return "AIR";
        for (ItemProvider library : itemDetectionArray) {
    ...
    public ItemStack build(Player player, ItemBuilder builder, Map<String, String> placeholders) {
        ...
        RtagItem nbtItem = new RtagItem(temp);
    ...
    public ItemStack getItemStackAppearance(Player player, String material) {
        if (material != null) {
            ...
            if (itemStack != null) {
                ItemStack cloned = new ItemStack(itemStack.getType());
     ...
     *
     * @param itemProvider The item library to register.
     * @return True if the item library was successfully registered, false if it already exists.
     */
    @Override
    public boolean registerItemLibrary(ItemProvider itemProvider) {
        if (itemLibraryMap.containsKey(itemProvider.identification())) return false;
        itemLibraryMap.put(itemProvider.identification(), itemProvider);
        this.resetItemDetectionOrder();
        return true;
    }
    ...
    @Override
    public void dropItem(Player player, Location hookLocation, Location playerLocation, ItemStack item, PlayerContext playerContext) {
        if (item.getType() == Material.AIR) {
            return;
        }

        if (CFConfig.enableFishingBag && plugin.getBagManager().doesBagStoreLoots() && RequirementManager.isRequirementMet(playerContext, plugin.getBagManager().getCollectRequirements())) {
            ...
            if (cannotPut != item.getAmount()) {
                ActionManager.triggerActions(playerContext, plugin.getBagManager().getCollectLootActions());
            }
            // all are put
            if (cannotPut == 0) {
                return;
            ...
            ActionManager.triggerActions(playerContext, plugin.getBagManager().getBagFullActions());
        }
    ...
    public static class CFBuilder implements ItemBuilder {
        ...
        public ItemBuilder customModelData(int value) {
            ...
            editors.put("custom-model-data", (player, nbtItem, placeholders) -> nbtItem.set(value, "CustomModelData"));
            return this;
        }
        ...
        public ItemBuilder tag(boolean tag, String type, String id) {
            editors.put("tag", (player, nbtItem, placeholders) -> {
                if (!tag) return;
                nbtItem.set(type, "CustomFishing", "type");
                nbtItem.set(id, "CustomFishing", "id");
            });
            return this;
        }
        ...
        public ItemBuilder placeable(boolean placeable) {
            editors.put("placeable", (player, nbtItem, placeholders) -> {
                if (!placeable) return;
                nbtItem.set((byte) 1, "CustomFishing", "placeable");
            });
            return this;
        }
        ...
        public ItemBuilder enchantment(List<Pair<String, Short>> enchantments, boolean store) {
            ...
            editors.put("enchantment", (player, nbtItem, placeholders) -> {
                ...
                for (Pair<String, Short> pair : enchantments) {
                    ...
                    TagCompound.set(nbtCompound, "id", pair.left());
                    TagCompound.set(nbtCompound, "lvl", pair.right());
                    TagList.add(nbtList, nbtCompound);
                }
                nbtItem.set(nbtList, store ? "StoredEnchantments" : "Enchantments");
            });
            return this;
        }
...
//                }
        ...
        @Override
        public ItemBuilder enchantmentPool(List<Pair<Integer, MathValue>> amountPairs, List<Pair<Pair<String, Short>, MathValue>> enchantments, boolean store) {
...
//                }
        ...
        public ItemBuilder maxDurability(int max) {
            ...
            editors.put("durability", (player, nbtItem, placeholders) -> {
                nbtItem.set(max, "CustomFishing", "max_dur");
                nbtItem.set(max, "CustomFishing", "cur_dur");
            });
            return this;
        }
        ...
        public ItemBuilder price(float base, float bonus) {
            ...
            editors.put("price", (player, nbtItem, placeholders) -> {
                ...
                double price;
                price = BukkitCustomFishingPlugin.get().getMarketManager().getFishPrice(
                        player,
                        placeholders
                );
                nbtItem.set(price, "Price");
        ...
        public ItemBuilder size(Pair<Float, Float> size) {
            ...
            editors.put("size", (player, nbtItem, placeholders) -> {
                ...
                nbtItem.set(random, "CustomFishing", "size");
        ...
        public ItemBuilder stackable(boolean stackable) {
            ...
            editors.put("stackable", (player, nbtItem, placeholders) -> {
                nbtItem.set(UUID.randomUUID(), "CustomFishing", "stackable");
            });
            return this;
        }
        ...
        public ItemBuilder preventGrabbing(boolean prevent) {
            ...
            editors.put("grabbing", (player, nbtItem, placeholders) -> {
                nbtItem.set(placeholders.get("player"), "owner");
            });
            return this;
        }
        ...
        public ItemBuilder head(String base64) {
            ...
            editors.put("head", (player, nbtItem, placeholders) -> {
                nbtItem.set(UUID.nameUUIDFromBytes(id.getBytes()), "SkullOwner", "Id");
        ...
        public ItemBuilder randomDamage(boolean damage) {
            ...
            editors.put("damage", (player, nbtItem, placeholders) -> {
                if (nbtItem.hasTag("CustomFishing")) {
                    int i = nbtItem.get("CustomFishing", "max_dur");
                    if (i != 0) {
                        int dur = ThreadLocalRandom.current().nextInt(i);
                        nbtItem.set(dur, "CustomFishing", "max_dur");
                        nbtItem.set((int) (nbtItem.getItem().getType().getMaxDurability() * ((double) dur / i)), "Damage");
                    } else {
                        nbtItem.set(ThreadLocalRandom.current().nextInt(nbtItem.getItem().getType().getMaxDurability()), "Damage");
                    }
                } else {
                    nbtItem.set(ThreadLocalRandom.current().nextInt(nbtItem.getItem().getType().getMaxDurability()), "Damage");
                }
    ...
    public void onPickUp(PlayerAttemptPickupItemEvent event) {
        ...
        if (!Objects.equals(nbtItem.get("owner"), event.getPlayer().getName())) {
            event.setCancelled(true);
        } else {
            nbtItem.remove("owner");
            nbtItem.load();
...
//        itemStack.setItemMeta(nbtItem.getItem().getItemMeta());
    ...
    public void onConsumeItem(PlayerItemConsumeEvent event) {
        ...
        if (loot != null) {
            PlayerContext playerContext = new PlayerContext(event.getPlayer());
            if (!loot.disableGlobalAction())
                GlobalSettings.triggerLootActions(ActionTrigger.CONSUME, playerContext);
            loot.triggerActions(ActionTrigger.CONSUME, playerContext);
        }
    }
...
//        event.setResult(nbtItem.getItem());
    ...
    public void onMending(PlayerItemMendEvent event) {
        ItemStack itemStack = event.getItem();
        RtagItem nbtItem = new RtagItem(itemStack);
        if (!nbtItem.hasTag("CustomFishing")) return;
        event.setCancelled(true);
    ...
    public void onInteractWithItems(PlayerInteractEvent event) {
        ...
        Loot loot = plugin.getLootManager().getLoot(id);
        if (loot != null) {
            loot.triggerActions(ActionTrigger.INTERACT, playerContext);
            return;
        }
    ...
    public void onPlaceBlock(BlockPlaceEvent event) {
        ...
        if (nbtItem.hasTag("CustomFishing")) {
            ...
            if (!nbtItem.hasTag("CustomFishing", "placeable") || (byte) nbtItem.get("CustomFishing", "placeable") != 1) {
                event.setCancelled(true);
                return;
            }
