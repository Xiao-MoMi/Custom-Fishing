...
public class ActionManagerImpl implements ActionManager {
    ...
    public ActionManagerImpl(BukkitCustomFishingPlugin plugin) {
        this.plugin = plugin;
    ...
    private void registerMendingAction() {
        registerAction("mending", (args, chance) -> {
            ...
            return condition -> {
                if (Math.random() > chance) return;
                if (BukkitCustomFishingPlugin.get().getVersionManager().isSpigot()) {
        ...
        registerAction("give-item", (args, chance) -> {
            if (args instanceof ConfigurationSection section) {
                ...
                return condition -> {
                    if (Math.random() > chance) return;
                    Player player = condition.getPlayer();
                    ItemUtils.giveItem(player, Objects.requireNonNull(BukkitCustomFishingPlugin.get().getItemManager().buildAnyPluginItemByID(player, id)), amount);
                };
            } else {
