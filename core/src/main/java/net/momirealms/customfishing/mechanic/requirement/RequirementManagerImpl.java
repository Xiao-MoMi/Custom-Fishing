...
public class RequirementManagerImpl implements RequirementManager {
    ...
    public RequirementManagerImpl(BukkitCustomFishingPluginImpl plugin) {
        this.plugin = plugin;
     ...
     */
    @Override
    public boolean registerRequirement(@NotNull String type, @NotNull RequirementFactory requirementFactory) {
    ...
    public HashMap<String, Double> getLootWithWeight(PlayerContext playerContext) {
        return getString2DoubleMap(playerContext, conditionalLootsMap);
    }

    public HashMap<String, Double> getGameWithWeight(PlayerContext playerContext) {
        return getString2DoubleMap(playerContext, conditionalGamesMap);
    }

    /**
     * Retrieves a mapping of strings to doubles based on conditional elements and a player's condition.
     *
     * @param playerContext The player's condition.
     ...
     */
    @NotNull
    private HashMap<String, Double> getString2DoubleMap(PlayerContext playerContext, LinkedHashMap<String, ConditionalElement> conditionalGamesMap) {
        ...
        while (!lootQueue.isEmpty()) {
            ...
            for (ConditionalElement loots : currentLootMap.values()) {
                if (RequirementManager.isRequirementMet(playerContext, loots.getRequirements())) {
    ...
    @Override
    @Nullable
    public RequirementFactory getRequirementFactory(@NotNull String type) {
    ...
    private void registerSeasonRequirement() {
        registerRequirement("season", (args, actions, advanced) -> {
            ...
            return condition -> {
                SeasonProvider seasonProvider = plugin.getIntegrationManager().getSeasonInterface();
                if (seasonProvider == null) return true;
                String season = seasonProvider.getSeason(condition.getLocation().getWorld());
    ...
    private void registerPluginLevelRequirement() {
        registerRequirement("plugin-level", (args, actions, advanced) -> {
            if (args instanceof ConfigurationSection section) {
                ...
                return condition -> {
                    LevelerProvider levelerProvider = plugin.getIntegrationManager().getLevelPlugin(pluginName);
                    ...
                    if (levelerProvider.getLevel(condition.getPlayer(), target) >= level)
                        return true;
                    if (advanced) triggerActions(actions, condition);
                    return false;
                };
     ...
     * @param playerContext The condition associated with the actions.
     */
    private void triggerActions(List<Action> actions, PlayerContext playerContext) {
        if (actions != null)
